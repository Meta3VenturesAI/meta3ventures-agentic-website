...
export const validateEnvironment = () => {
  ...
  const missingOptional = optionalEnvVars.filter(envVar => !import.meta.env[envVar]);
  
  if (missing.length > 0) {
    console.error('Missing required environment variables:', missing);
    console.error('Please set these variables in your .env file for full functionality.');
  }

  if (missingOptional.length > 0) {
    console.warn('Missing optional environment variables:', missingOptional);
    console.warn('Some form features may use fallback configurations.');
  }
